...
export default function PortfolioForm({ portfolioId }: { portfolioId?: string }) {
  ...
  async function fetchPortfolio() {
    try {
      ...
      // Convert legacy impact objects to simple strings
      const rawImpact = data.impact || []
      const formattedImpact = rawImpact.map((item: any) => {
        if (typeof item === 'string') return item
        if (typeof item === 'object' && item !== null) {
          // Legacy format conversion: "40% Sales Increase"
          const val = item.value || ''
          const unit = item.unit || ''
          const title = item.title || ''
          return `${val}${unit} ${title}`.trim()
        }
        return ''
      }).filter(Boolean)
      ...
      setFormData({
        ...
        impact: formattedImpact, // Use the converted simple string array
            ...
            ? oldSubsteps.map((sub: any) => {
              ...
              if (typeof sub === 'object' && sub !== null && 'title' in sub) {
                return {
                  ...
                  images: sub.images || [],
                  layout: sub.layout || 'grid'
                }
              }
              // If it's a string, convert to new format
              return { title: sub, description: '', images: [], layout: 'grid' }
            })
            : []
  ...
  const tabs = [
    ...
    { id: "impact", label: "Detailed Outcomes", icon: LayoutGrid },
  ]
              ...
              <div className="grid grid-cols-1 gap-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mb-3">What did you do specifically?</p>
                      ...
                      {formData.responsibilities.map((resp, idx) => (
                        <div key={idx} className="flex gap-2 group">
                          <input
                            ...
                            className="flex-1 px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-sm focus:ring-1 focus:ring-indigo-500"
                            placeholder="Developed the API..."
                          />
                          <button type="button" onClick={() => setFormData({ ...formData, responsibilities: formData.responsibilities.filter((_, i) => i !== idx) })} className="text-gray-400 hover:text-red-500 p-2 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 className="w-4 h-4" /></button>
                        </div>
                      ))}
                      <button type="button" onClick={() => setFormData({ ...formData, responsibilities: [...formData.responsibilities, ""] })} className="text-sm text-indigo-600 font-medium flex items-center gap-2 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 px-3 py-2 rounded-lg transition-colors w-fit"><Plus className="w-3.5 h-3.5" /> Add Responsibility</button>
                    </div>
                  </div>

                  {/* Right: Impact & Results */}
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Key Achievements (Metrics)</label>
                    <p className="text-xs text-gray-500 mb-3">Quantifiable results for the Bento Grid.</p>
                    <div className="space-y-2">
                      {formData.impact.map((item, idx) => (
                        <div key={idx} className="flex gap-2 group">
                          <input
                            ...
                            className="flex-1 px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-sm font-medium text-indigo-600 dark:text-indigo-400 focus:ring-1 focus:ring-indigo-500"
                            placeholder="e.g. 40% Increase in Sales"
                          />
                          <button type="button" onClick={() => setFormData({ ...formData, impact: formData.impact.filter((_, i) => i !== idx) })} className="text-gray-400 hover:text-red-500 p-2 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 className="w-4 h-4" /></button>
                        </div>
                      ))}
                      <button type="button" onClick={() => setFormData({ ...formData, impact: [...formData.impact, ""] })} className="text-sm text-indigo-600 font-medium flex items-center gap-2 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 px-3 py-2 rounded-lg transition-colors w-fit"><Plus className="w-3.5 h-3.5" /> Add Metric</button>
                    </div>
                  </div>
                </div>
              </div>
                            ...
                            {(step.substeps || []).map((substep, subIdx) => (
                              <div key={subIdx} className="p-4 rounded-lg bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 relative group">
                                <div className="space-y-3">
                                  ...
                                  <div>
                                    <label className="block text-xs text-gray-500 mb-1">Image Layout</label>
                                    <div className="flex bg-gray-100 dark:bg-gray-900 rounded-lg p-1 w-fit border border-gray-200 dark:border-gray-700">
                                      <button
                                        type="button"
                                        onClick={() => {
                                          setFormData(prev => {
                                            ...
                                            newSubsteps[subIdx] = { ...newSubsteps[subIdx], layout: 'grid' }
                                            newSteps[idx] = { ...newSteps[idx], substeps: newSubsteps }
                                            return { ...prev, project_steps: newSteps }
                                          })
                                        }}
                                        className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all flex items-center gap-1.5 ${substep.layout !== 'stack'
                                      ...
                                      <button
                                        ...
                                        onClick={() => {
                                          setFormData(prev => {
                                            const newSteps = [...prev.project_steps]
                                            const newSubsteps = [...(newSteps[idx].substeps || [])]
                                            newSubsteps[subIdx] = { ...newSubsteps[subIdx], layout: 'stack' }
                                            newSteps[idx] = { ...newSteps[idx], substeps: newSubsteps }
                                            return { ...prev, project_steps: newSteps }
                                          })
                                        }}
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-8 border border-gray-100 dark:border-gray-800 shadow-sm space-y-8">
            ...
            <div>
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-xl font-bold">Project Outcomes (Deep Dive)</h3>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    Add detailed success stories with descriptions. These appear as rich cards at the bottom of the page.
                    For simple metrics (e.g. "40% faster"), use the Overview tab instead.
                  </p>
                </div>
                <div className="w-1/3">
