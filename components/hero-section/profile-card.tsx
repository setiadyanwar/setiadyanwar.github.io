...
function FloatingIcon({ icon, position, delay = 0 }: FloatingIconProps) {
    ...
    const positions = {
        "top-right": "absolute -top-2 right-12 md:top-10 md:right-0",
        "bottom-left": "absolute -bottom-2 left-12 md:bottom-20 md:left-0",
        "top-left": "absolute top-0 left-12 md:top-2/3 md:-left-6",
        "mid-right": "absolute top-1/2 right-12 md:top-1/3 md:-right-6"
    };
    ...
    return (
        <motion.div
            className={`${positions[position]} w-8 h-8 md:w-12 md:h-12 bg-white/30 dark:bg-black/30 backdrop-blur-2xl backdrop-saturate-150 rounded-full flex items-center justify-center shadow-xl border border-white/30 dark:border-white/20 z-20`}
            animate={animation}
            transition={{
...
export default function ProfileCard({ isMobile: forcedMobile = false }: ProfileCardProps) {
    return (
        <motion.div
            layoutId="profile-card-main"
        ...
        >
            {/* Mobile/Tablet View */}
            <div className={`relative flex justify-center items-center h-full w-full ${forcedMobile ? 'flex' : 'lg:hidden'}`}>
            ...
            {/* Desktop View */}
            <div className={`relative ${forcedMobile ? 'hidden' : 'hidden lg:block'} h-full w-full max-w-[400px] mx-auto`}>
                <div
                    className="absolute inset-0 rounded-3xl shadow-2xl shadow-gray-200/30 dark:shadow-black/30 bg-white/10 dark:bg-black/10 backdrop-blur-xl backdrop-saturate-150"
                    style={{
                ...
                {/* Floating icons for desktop - outside the overflow hidden container */}
                <FloatingIcon icon="code" position="top-right" />
                <FloatingIcon icon="palette" position="bottom-left" delay={0.5} />
                <FloatingIcon icon="monitor" position="top-left" delay={1.5} />
                ...
                {/* Setiady Anwar badge */}
                <div
                    className="absolute bottom-8 left-20 transform -translate-x-1/2 w-fit rounded-2xl py-3 px-6 shadow-2xl backdrop-blur-2xl backdrop-saturate-150 border border-white/30 dark:border-white/20 z-20"
                    style={{
                ...
                <motion.div
                    className="absolute top-1/3 -right-6 w-24 h-24 flex items-center justify-center z-20"
